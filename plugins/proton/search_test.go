...
func TestSearch_RefusesEmptyMembership(t *testing.T) {
	...
	if _, err := p.Search(t.Context(), &toposv1.SearchRequest{Query: "gamma", MatchFields: map[string]*toposv1.StringList{"labels": {Values: []string{"house"}}}}); status.Code(err) != codes.InvalidArgument {
